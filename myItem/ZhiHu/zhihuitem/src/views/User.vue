          ...
          <div class="wmain">
            ...
            <div class="content">
              ...
              <div class="contenFooter">
                ...
                <div class="contenFooter_btn2">
                  <button @click="editUser">编辑个人资料</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async getQusetion() {
      let user = window.sessionStorage.getItem("user");
      user = JSON.parse(user);
      const { data: res } = await this.$http.get(`/question_user/${user.id}`);
      console.log(111, res);
    ...
    async getanswerList() {
      let user = window.sessionStorage.getItem("user");
      user = JSON.parse(user);
      const { data: res } = await this.$http.get(`/answer_user/${user.id}`);
