$(function () {
    ...
    function run() {
        ...
        $('#carousel_fixed .content .take_Turns_pic .circle li').eq(c).css({'background': 'white'}).siblings('li').css({'background': 'rgba(0,0,0,0)'});
    };
    ...
    $('#carousel_fixed .content .take_Turns_pic .left_btn').click(function () {
        ...
        $('#carousel_fixed .content .take_Turns_pic .circle li').eq(c).css({'background': 'red'}).siblings('li').css({'background': 'rgba(0,0,0,0)'}).css({'opacity':"0"});
    });
    //右按钮点击事件
...
// 右边导航栏锚点效果
// var btn=document.getElementById('high-down');
// 	//timer需要放在最外面
// 	var timer=null;
// 	var istop=true;
// 	//屏幕可视化高度
